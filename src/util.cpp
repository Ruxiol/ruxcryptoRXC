...
const char * const BITCOIN_CONF_FILENAME = "ruxcrypto.conf";
const char * const BITCOIN_PID_FILENAME = "ruxcryptod.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "ruxcrypto" is a composite category enabling all RuxCrypto-related debug output
                if(ptrCategory->count(std::string("ruxcrypto"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RuxCrypto
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RuxCrypto
    // Mac: ~/Library/Application Support/RuxCrypto
    // Unix: ~/.ruxcrypto
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RuxCrypto";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RuxCrypto";
#else
    // Unix
    return pathRet / ".ruxcrypto";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ruxcrypto.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ruxcrypto.conf
