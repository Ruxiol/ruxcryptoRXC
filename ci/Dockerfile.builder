...
# ruxcrypto_hash
RUN git clone https://github.com/ruxcryptopay/ruxcrypto_hash
RUN cd ruxcrypto_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ruxcrypto
RUN useradd -u ${USER_ID} -g ruxcrypto -s /bin/bash -m -d /ruxcrypto ruxcrypto
...
RUN mkdir /ruxcrypto-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ruxcrypto-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /ruxcrypto-src
