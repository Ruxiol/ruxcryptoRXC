...
void CChainLocksHandler::ProcessNewChainLock(NodeId from, const llmq::CChainLockSig& clsig, const uint256& hash)
{
    ...
    if (!quorumSigningManager->VerifyRecoveredSig(GetActiveQuorumType(Params().GetConsensus().llmqChainLocks), clsig.nHeight, requestId, msgHash, clsig.sig)) {
...
void CChainLocksHandler::TrySignChainTip()
{
    ...
    quorumSigningManager->AsyncSignIfMember(GetActiveQuorumType(Params().GetConsensus().llmqChainLocks), requestId, msgHash);
}
