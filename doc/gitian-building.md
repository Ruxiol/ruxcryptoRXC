...
*Setup instructions for a Gitian build of RuxCrypto Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the RuxCrypto
...
- [Building RuxCrypto Core](#building-ruxcrypto-core)
...
Clone the git repositories for RuxCrypto Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ruxcryptopay/ruxcrypto
...
in the RuxCrypto Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building RuxCrypto Core
----------------

To build RuxCrypto Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the RuxCrypto Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ruxcrypto/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ruxcryptopay/ruxcrypto
...
For example:
```bash
URL=https://github.com/crowning-/ruxcrypto.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ruxcrypto=${COMMIT} --url ruxcrypto=${URL} ../ruxcrypto/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ruxcrypto=${COMMIT} --url ruxcrypto=${URL} ../ruxcrypto/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ruxcrypto=${COMMIT} --url ruxcrypto=${URL} ../ruxcrypto/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ruxcrypto git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ruxcrypto/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ruxcrypto
SIGPATH=/some/root/path/ruxcrypto-detached-sigs

./bin/gbuild --url ruxcrypto=${BTCPATH},signature=${SIGPATH} ../ruxcrypto/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ruxcrypto-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ruxcrypto-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ruxcrypto-osx-build.assert
```
...
[ruxcrypto/gitian.sigs](https://github.com/ruxcryptopay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
