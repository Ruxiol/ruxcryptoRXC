...
// which are part of the RuxCrypto repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="ruxcrypto-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /ruxcrypto-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "ruxcryptopay-ruxcrypto/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from ruxcryptopay-ruxcrypto/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'ruxcryptopay-ruxcrypto/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from ruxcryptopay-ruxcrypto/develop"
            }
          }

          if (hasCache) {
            sh "cd /ruxcrypto-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /ruxcrypto-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /ruxcrypto-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /ruxcrypto-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /ruxcrypto-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /ruxcrypto-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /ruxcrypto-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
